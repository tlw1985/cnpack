...
function country_code_by_addr($ip)
{
  ...
  while (true)
  {
    ...
    $buf = fread($fd, 4);
    $startip = implode('', unpack('L', $buf));
    if($startip < 0)
      $startip += pow(2, 32);
      
    $buf = fread($fd, 4);
    $endip = implode('', unpack('L', $buf));
    if($endip < 0)
      $endip += pow(2, 32);
...
{
  global $g_country_list, $COUNTRY_CODES, $COUNTRY_NAMES;
  if (count($g_country_list) == 0)
  {
    for ($i = 0; $i < count($COUNTRY_CODES); $i++)
      $g_country_list[$COUNTRY_CODES[$i]] = $COUNTRY_NAMES[$i];
  }
  $code = $g_country_list[$code];
  if ($code == '')
    $code = $COUNTRY_NAMES[0];
  return $code;
}

function lang_name_by_id($lang)
{
  global $g_lang_list, $LANG_IDS, $LANG_NAMES;
  if (count($g_lang_list) == 0)
  {
    for ($i = 0; $i < count($LANG_IDS); $i++)
      $g_lang_list[$LANG_IDS[$i]] = $LANG_NAMES[$i];
  }
  $name = $g_lang_list[$lang];
  if ($name == '')
    $name = $LANG_NAMES[0];
  return $name;
}
